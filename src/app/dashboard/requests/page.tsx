...
import { PlusCircle, FilePlus, Loader2, Check, MoreVertical, History, RefreshCcw, AlertTriangle, Undo2, PackageCheck, Truck, XCircle, Home, Pencil, FilterX, CalendarIcon, Users, User as UserIcon, ChevronLeft, ChevronRight, Clock, FileDown, Layers, Lightbulb } from 'lucide-react';
...
export default function PurchaseRequestPage() {
    ...
    const {
        ...
        currentPage, pageSize, totalItems, requestSettings, newRequest, requestToEdit,
    ...
    return (
        <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
            ...
            <div className="space-y-4">
             ...
             {totalItems > pageSize && (<div className="flex items-center justify-center space-x-2 py-4"><Button variant="outline" size="sm" onClick={() => actions.setCurrentPage(p => p - 1)} disabled={currentPage === 0}><ChevronLeft className="mr-2 h-4 w-4" />Anterior</Button><span className="text-sm text-muted-foreground">Página {currentPage + 1} de {Math.ceil(totalItems / pageSize)}</span><Button variant="outline" size="sm" onClick={() => actions.setCurrentPage(p => p + 1)} disabled={(currentPage + 1) * pageSize >= totalItems}>Siguiente<ChevronRight className="ml-2 h-4 w-4" /></Button></div>)}
